...
- **Cold-cache fork coordination (#24)**: parallel oxlint isolates that all miss the cache for the same CSS would each fork their own precompute child, exhausting memory on constrained hosts (WSL `ENOMEM`). `computeWithLock` in `sync-loader.ts` gates the fork behind a `<contentHash>.lock` file (atomic `openSync(..., 'wx')`): the winner forks + writes the cache, the rest busy-wait via an Atomics sync-sleep for the `<contentHash>.json`. A lock older than `timeout + 30s` (a holder that died mid-compute) is reclaimed; a non-writable cache dir degrades to an uncoordinated compute rather than spinning. `cacheArtifactPaths(cssPath)` exposes the json/lock paths for tests.
